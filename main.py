...
@app.post("/submit")
def submit_answers(submission: Submission, token: str = Depends(verify_token), db: Session = Depends(get_db)):
    total_score = 0
    max_score = 0
    qs = {q.id: q for q in db.query(Question).all()}

    for ans in submission.answers:
        q = qs.get(ans.question_id)
        if not q:
            continue
        if ans.selected == q.answer and ans.time_taken <= 30:
            total_score += q.weight
        max_score += q.weight

    iq = int(85 + (total_score / max_score) * 60)
    db.add(Result(user_token=token, score=total_score, max_score=max_score, iq=iq))
    db.commit()
    return {"score": total_score, "max_score": max_score, "estimated_iq": iq}
